...
interface Message {
  ...
  senderId: string;
  senderType: 'place' | 'admin' | 'customer';
  senderName: string;
  message: string;
  timestamp: string;
  read: boolean;
...
const PlaceChat: React.FC<PlaceChatProps> = ({ place, onBack }) => {
  ...
  // Generate conversation ID (place-{placeId}-admin)
  const conversationId = place ? `place-${place.$id || place.id}-admin` : '';
  ...
  const fetchMessages = async () => {
    ...
    try {
      console.log('📥 Fetching messages for place:', place.$id || place.id);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50">
        ...
        <main className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-2xl shadow-lg p-12 text-center text-white">
            ...
            <button
              onClick={onBack}
              className="px-8 py-3 bg-white text-orange-600 font-bold rounded-xl hover:bg-gray-100 transition-colors shadow-lg"
            >
              Upgrade to Premium
            </button>
          </div>
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {loading ? (
              ...
              <>
                {messages.map((msg) => {
                  const isOwnMessage = msg.senderType === 'place';
                  return (
                    <div
                      key={msg.$id}
          ...
          <div className="border-t border-gray-200 p-4 bg-gray-50">
            <div className="flex items-end gap-3">
              <textarea
                ...
                className="flex-1 resize-none border-2 border-gray-300 rounded-xl px-4 py-3 focus:border-orange-500 focus:outline-none transition-colors text-sm min-h-[44px] max-h-32"
                rows={1}
                disabled={sending}
              />
              <button
