...
const PackageTermsPage: React.FC<PackageTermsPageProps> = ({ onBack, onNavigate }) => {
  const [storedLanguage, setStoredLanguage] = useState<'en' | 'id'>(getStoredLanguage());
  ...
  useEffect(() => {
    ...
    const handleStorageChange = () => setStoredLanguage(getStoredLanguage());
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 pt-8 pb-6 text-center">
        <p className="text-sm text-gray-500 mb-2">
          {isPro ? (language === 'id' ? 'Paket Pro' : 'Pro Plan') : (language === 'id' ? 'Paket Plus' : 'Plus Plan')}
          <span className="mx-2">•</span>
          <span className="text-orange-600 font-medium">{isPro ? (language === 'id' ? 'Bayar Per Lead' : 'Pay Per Lead') : (language === 'id' ? '0% Komisi' : '0% Commission')}</span>
        </p>
        <h1 className="text-2xl font-bold text-gray-900 mb-3">{language === 'id' ? 'Syarat & Ketentuan' : 'Terms & Conditions'}</h1>
        <p className="text-gray-600 text-sm max-w-md mx-auto">
          {isPro 
            ? (language === 'id' 
              ? 'Keanggotaan Pro menghubungkan Anda dengan pelanggan IndaStreet. Dapatkan 70% dari setiap booking yang dikonfirmasi.'
              : 'Pro membership connects you to IndaStreet customers. Earn 70% of every confirmed booking.')
            : (language === 'id'
              ? 'Keanggotaan Plus memberikan Anda kontrol penuh. Biaya bulanan tetap, simpan 100% booking, penempatan premium.'
