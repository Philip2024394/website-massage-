...
async function declineBooking(bookingId: string, providerId: string, reason?: string) {
  const env = getEnv();
...
export default async function (req: any, res: any): Promise<void> {
  try {
    const { bookingId, providerId, token, reason } = req.query || {};
    ...
    const result = await declineBooking(bookingId, providerId, reason);
