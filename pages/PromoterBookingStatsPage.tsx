...
const PromoterBookingStatsPage: React.FC<{ t?: any; onBack?: () => void; onNavigate?: (p: any) => void }> = ({ t, onBack, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try { 
        ...
        try {
          if (id) { const prof = await promoterService.getProfile(id); if (mounted) setIsActive(!!((prof as any).isActive ?? (prof as any).active)); }
        } catch { if (mounted) setIsActive(true); }
      } catch { if (mounted) { setUserId(''); setIsActive(true); } }
    })();
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      if (!userId) { setRows([]); setTotals({ pending: 0, approved: 0, paid: 0, amountPending: 0, amountApproved: 0, amountPaid: 0 }); return; }
      setLoading(true);
      try {
        const atts = await affiliateAnalyticsService.getAttributionsByCode(userId);
        if (!mounted) return;
        setRows(atts);
