...
const AgentDashboardPage: React.FC<AgentDashboardPageProps> = ({ agent, onLogout, t, isAdminView = false, onStopImpersonating, messages = [], onSendMessage, onMarkMessagesAsRead, onSaveProfile }) => {
    ...
    const [metrics, setMetrics] = useState<MonthlyAgentMetrics[]>([]);
    ...
    const idrFormatter = useMemo(() => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }), []);
        ...
        interface AgentVisit {
            id: string;
            date: string; // ISO date of visit
            entityType: 'therapist' | 'place';
            entityName: string;
            location: string;
            outcome: 'positive' | 'negative' | 'need_info' | 'need_time';
            callbackDate?: string; // ISO date for follow-up
            callbackTime?: string; // HH:MM
            notes?: string;
            createdAt: string;
        }
        const [visits, setVisits] = useState<AgentVisit[]>([]);
        // New visit form fields
        const [visitEntityType, setVisitEntityType] = useState<'therapist' | 'place'>('therapist');
        const [visitEntityName, setVisitEntityName] = useState('');
        const [visitLocation, setVisitLocation] = useState('');
        const [visitOutcome, setVisitOutcome] = useState<'positive' | 'negative' | 'need_info' | 'need_time'>('positive');
        const [visitCallbackDate, setVisitCallbackDate] = useState('');
        const [visitCallbackTime, setVisitCallbackTime] = useState('');
        const [visitNotes, setVisitNotes] = useState('');

        const handleAddVisit = async () => {
            if (!visitEntityName.trim()) return;
            try {
                setErrorMsg(null);
                const nowIso = new Date().toISOString();
                // Map UI outcome to status & membershipAgreed defaults
                const status: 'pending' | 'completed' | 'followup_required' = visitOutcome === 'positive'
                    ? 'completed'
                    : (visitOutcome === 'negative' ? 'pending' : 'followup_required');
                const created = await agentVisitService.createVisit({
                    agentId: agentDoc.$id || agentDoc.agentId,
                    agentName: agentDoc.name,
                    agentCode: agentDoc.agentCode,
                    providerName: visitEntityName.trim(),
                    providerType: visitEntityType,
                    whatsappNumber: '',
                    visitDate: nowIso,
                    location: {
                        lat: 0,
                        lng: 0,
                        address: visitLocation.trim(),
                        timestamp: nowIso
                    },
                    meetingNotes: visitNotes.trim() || '',
                    callbackDate: visitCallbackDate || undefined,
                    membershipAgreed: 'none',
                    status,
                    createdAt: nowIso,
                    updatedAt: nowIso
                });
                setRemoteVisits(prev => [created, ...prev]);
                // Reset form
                setVisitEntityName('');
                setVisitLocation('');
                setVisitOutcome('positive');
                setVisitCallbackDate('');
                setVisitCallbackTime('');
                setVisitNotes('');
            } catch (err: any) {
    ...
    // Profile state
    // Personal info
    const [profileImage, setProfileImage] = useState(agent.profileImage || '');
    const [fullName, setFullName] = useState(agent.name || '');
    const [age, setAge] = useState<number | ''>(typeof agent.age === 'number' ? agent.age : '');
    const [religion, setReligion] = useState(agent.religion || '');
    ...
    const nameMismatch = useMemo(() => !!(fullName && bankAccountName && fullName.trim() !== bankAccountName.trim()), [fullName, bankAccountName]);
