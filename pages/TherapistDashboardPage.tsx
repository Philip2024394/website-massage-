...
interface TherapistDashboardPageProps {
    onSave: (data: Omit<Therapist, 'id' | 'isLive' | 'status' | 'rating' | 'reviewCount'>) => void;
    onBack: () => void;
...
const TherapistDashboardPage: React.FC<TherapistDashboardPageProps> = ({ onSave, onBack, therapist, t }) => {
    ...
    useEffect(() => {
        if (therapist) {
            ...
            setMassageTypes(therapist.massageTypes.join(', '));
    ...
    const handleSave = () => {
        onSave({
            ...
            whatsappNumber,
            pricing,
            massageTypes: massageTypes.split(',').map(s => s.trim()),
            location,
                ...
                <div>
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-700">{t.massageTypesLabel}</label>
                     {renderInput(massageTypes, setMassageTypes, TagIcon, 'Swedish, Deep Tissue')}
                </div>
                 <div>
