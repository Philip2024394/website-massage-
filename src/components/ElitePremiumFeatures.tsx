 ...
 * - amenities (or default icons) → "Spa Amenities"
 * - languagesSpoken[] → shown on profile card under description (with flags) when member selects in dashboard
 * - paymentMethods (or default icons) → "Payment Methods"
 * - safetyFeatures[] → "Safety & Comfort"
 * - ambianceTags[] → mood tags + couplesTreatmentAvailable → "Couple's Treatment"
 * - therapists[] → "Featured Therapists"
 * - testimonials[] → "Customer Reviews"
 * - giftCardAvailable → "Gift This Spa" button
 * - viewingNow, lastBookedMinutesAgo → urgency strip (optional/demo)
 */

import React, { useState, useEffect } from 'react';
import {
  Sparkles, ShowerHead, Coffee, Wifi, Lock, CreditCard, Banknote, Smartphone,
  Building2, ShieldCheck, Users, Gift, Star, Quote, Clock, Eye, Heart,
} from 'lucide-react';
...
const DEFAULT_AMBIANCE_TAGS = ['Relaxing', 'Modern', 'Professional'];

const DEFAULT_THERAPISTS: Therapist[] = [
  { id: '1', name: 'Sari Dewi', photo: 'https://ik.imagekit.io/7grri5v7d/therapist1.png', specialty: 'Traditional Balinese', yearsExperience: 8, rating: 4.9 },
  { id: '2', name: 'Putu Ayu', photo: 'https://ik.imagekit.io/7grri5v7d/therapist2.png', specialty: 'Deep Tissue & Sports', yearsExperience: 5, rating: 4.8 },
  { id: '3', name: 'Wayan Sinta', photo: 'https://ik.imagekit.io/7grri5v7d/therapist3.png', specialty: 'Aromatherapy & Relaxation', yearsExperience: 6, rating: 4.9 },
];
...
export default function ElitePremiumFeatures({
  ...
  const whatsIncluded = p.whatsIncluded || DEFAULT_WHATS_INCLUDED;
  const safetyFeatures = p.safetyFeatures || DEFAULT_SAFETY_FEATURES;
  const ambianceTags = p.ambianceTags || DEFAULT_AMBIANCE_TAGS;
  const therapists = p.therapists || DEFAULT_THERAPISTS;
  const testimonials = p.testimonials || DEFAULT_TESTIMONIALS;
  const viewingNow = p.viewingNow ?? 2;
  const lastBookedMinutesAgo = p.lastBookedMinutesAgo ?? 12;
  const couplesTreatment = p.couplesTreatmentAvailable ?? true;
    ...
    <section className={`mt-8 ${sectionCardClass}`}>
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100 text-gray-700 text-xs font-medium border border-gray-200">
            <Clock className="w-3.5 h-3.5 text-gray-500" />
            {isId ? `Terakhir dipesan ${lastBookedMinutesAgo} menit lalu` : `Last booked ${lastBookedMinutesAgo} min ago`}
          </span>
        </div>

        {/* ——— Ambiance + Couple's tags ——— */}
        <div className="flex flex-wrap items-center justify-center gap-2">
          {ambianceTags.map((tag: string, i: number) => (
            <span
              key={i}
              className="px-3 py-1.5 rounded-full bg-amber-50 text-amber-800 text-xs font-medium border border-amber-200"
            >
              {tag}
            </span>
          ))}
          {couplesTreatment && (
            <span className="px-3 py-1.5 rounded-full bg-amber-50 text-amber-800 text-xs font-medium border border-amber-200 inline-flex items-center gap-1.5">
              <Heart className="w-3.5 h-3.5 text-amber-600" />
              {isId ? 'Perawatan Pasangan' : "Couple's Treatment"}
            </span>
          )}
        </div>

        {/* ——— What's Included + Spa Amenities (grid on sm+) ——— */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl border-2 bg-orange-50/80 border-orange-400">
            <h4 className={`${sectionTitleClass} mb-3`}>
              <Sparkles className={sectionIconClass} />
          ...
          <div className="p-4 rounded-xl border-2 bg-orange-50/80 border-orange-400">
            ...
            <div className="grid grid-cols-4 gap-3">
              {AMENITY_CONFIG.map(({ icon: Icon, label, labelId }, i) => (
        ...
        {/* ——— Payment Methods (full width; Languages moved to profile card under description) ——— */}
        <div className="p-4 rounded-xl border border-gray-200 bg-gray-50/50">
          <h4 className={`${sectionTitleClass} mb-3`}>
            {isId ? 'Metode Pembayaran' : 'Payment Methods'}
          </h4>
          <div className="grid grid-cols-4 gap-3">
            {PAYMENT_CONFIG.map(({ icon: Icon, label, labelId }, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
        ...
        <div>
          <h4 className={`${sectionTitleClass} mb-3`}>
            ...
            {isId ? 'Terapis Unggulan' : 'Featured Therapists'}
          </h4>
          <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 snap-x snap-mandatory">
            {therapists.slice(0, 3).map((t: Therapist) => (
              <div
                key={t.id}
