...
export function PersistentChatWindow() {
  ...
  // Format countdown to MM:SS - SAFE VERSION with defensive guards
              ...
              try {
                return (
                  <SimpleBookingWelcome
                    ...
                    bookingCountdown={timerState.remainingSeconds}
                    ...
                    bookingDetails={{
                      ...
                      massageFor: chatState.currentBooking.massageFor,
                      locationType: chatState.currentBooking.locationType,
                      duration: chatState.selectedDuration || chatState.currentBooking.duration,
                      price: chatState.currentBooking.price || chatState.currentBooking.totalPrice
                    }}
                  />
                );
              } catch (error) {
                  ...
                  <div style={{ 
                    ...
                    <div style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px solid #d1d5db' }}>
                      <div style={{ fontWeight: 600, fontSize: '16px', color: '#111827', marginBottom: '4px' }}>
                        {chatState.therapist?.name || 'Therapist'}
                      </div>
              ...
              >
                ...
                <>
                  {messages.map((msg: ChatMessage) => {
                    ...
                    if (isSystem) {
                      // Hide green success notice when showing replacement search message
                      const isReplacementSearch = msg.message.includes('unfortunately is busy') || msg.message.includes('locate an alternative replacement');
                      const hasReplacementMessage = messages.some(m => 
                        m.senderType === 'system' && 
                        (m.message.includes('unfortunately is busy') || m.message.includes('locate an alternative replacement'))
                      );
                      const isBookingRequestSent = msg.message.includes('Booking request sent to therapist');
                      
                      // Don't render green booking notice if replacement search is active
                      if (isBookingRequestSent && hasReplacementMessage) {
                        return null;
                      }
                      ...
                      const isWarning = msg.message.includes('⚠️') || msg.message.includes('⏰') || isReplacementSearch;
