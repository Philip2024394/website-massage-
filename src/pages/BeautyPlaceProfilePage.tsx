/**
 * Beauty Place (Salon) Profile Page – City Places > Beauty
 * Same layout as Facial place profile; copy and labels adapted for beauty salons (nails, hair, makeup).
 */
...
export interface BeautyPlaceProfilePageProps {
    place: Place | null;
    placeId?: string;
    beautyPlaces?: Place[];
    onBack: () => void;
...
const DEFAULT_GALLERY = [
    { imageUrl: 'https://ik.imagekit.io/7grri5v7d/hait%20styles%20browns.png', header: 'Salon Space', description: 'Modern setup for nails, hair and makeup.' },
    { imageUrl: 'https://ik.imagekit.io/7grri5v7d/hait%20styles%20browns.png', header: 'Nail Station', description: 'Clean nail station with premium products.' },
];

const BeautyPlaceProfilePage: React.FC<BeautyPlaceProfilePageProps> = ({
    place: placeProp,
    placeId,
    beautyPlaces = [],
    onBack,
    ...
    const [loading, setLoading] = useState(!placeProp && !!(placeId || (typeof window !== 'undefined' && window.location.pathname.match(/\/profile\/beauty\/([^/]+)/))));
    ...
    useEffect(() => {
        ...
        const pathMatch = typeof window !== 'undefined' && window.location.pathname.match(/\/profile\/beauty\/([^/]+)/);
        const hashMatch = typeof window !== 'undefined' && (window.location.hash || '').match(/profile\/beauty\/([^/]+)/);
        const segment = (pathMatch && pathMatch[1]) || (hashMatch && hashMatch[1]);
        if (!segment && !placeId) {
            setLoading(false);
            return;
        }
        const slugFromName = (name: string) => (name || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
        const fromList = beautyPlaces.find((p: any) => {
        ...
        setLoading(false);
    }, [placeId, beautyPlaces, place, loading]);
    ...
    const heroImages = useMemo(() => {
        if (!place) return ['https://ik.imagekit.io/7grri5v7d/hait%20styles%20browns.png'];
        const list: string[] = [];
        ...
        if (list.length === 0) list.push('https://ik.imagekit.io/7grri5v7d/hait%20styles%20browns.png');
        return list;
    ...
    const galleryBlocks = useMemo(() => {
        ...
        return DEFAULT_GALLERY.slice(0, MAX_GALLERY_ITEMS);
    }, [place?.galleryImages, place]);

    const servicesList = useMemo(() => {
        if (!place) return [];
        const beauty = place.beautyServices;
        if (Array.isArray(beauty) && beauty.length > 0) return beauty;
        const facial = parseMassageTypes(place.facialTypes);
        return Array.isArray(facial) ? facial : [];
    }, [place?.beautyServices, place?.facialTypes, place]);

    const otherServices = useMemo(() => {
        if (!place) return ['Consultation', 'Nail Art', 'Hair Styling'];
        const amen = place.amenities;
        if (Array.isArray(amen) && amen.length > 0) return amen;
        return ['Consultation', 'Nail Art', 'Hair Styling'];
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">Loading salon profile...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-lg p-6 max-w-sm text-center">
                    <p className="text-gray-700 mb-4">Salon not found.</p>
            ...
            <div className="pt-[60px] max-w-4xl mx-auto pb-28">
                        ...
                        <div className="flex items-start gap-3">
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-amber-600 font-medium mt-1.5">
                                    {language === 'id' ? 'Fokus: Salon kecantikan – nails, hair & makeup' : 'Our focus: Beauty salon – nails, hair & makeup'}
                                </p>
                ...
                {(heroImages.length > 0 || galleryBlocks.length > 0) && (
                    <section className="mt-4 px-4">
                        <h2 className="text-sm font-semibold text-slate-700 mb-1">
                            {language === 'id' ? 'Foto salon' : 'Salon photos'}
                        </h2>
                ...
                <section className="mt-6 px-4 space-y-6">
                    <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                        ...
                        {language === 'id' ? 'Galeri foto salon' : 'Salon photos'}
                    </h2>
                    {galleryBlocks.map((block, index) => (
                        <div
                            key={index}
                ...
                <section className="mt-8 px-4">
                    <div className="bg-white rounded-2xl shadow-md border-2 border-slate-200 p-4">
                        ...
                        <p className="text-xs text-gray-600 mb-3 font-medium">
                            {language === 'id' ? 'Nails, hair, makeup & perawatan' : 'Nails, hair, makeup & treatments'}
                        </p>
                        {servicesList.length > 0 ? (
                            ...
                            <p className="text-gray-800 text-sm font-medium">
                                {language === 'id' ? 'Berbagai layanan salon kecantikan.' : 'Various beauty salon services.'}
                            </p>
                        )}
                    </div>
                </section>
...
export default BeautyPlaceProfilePage;
