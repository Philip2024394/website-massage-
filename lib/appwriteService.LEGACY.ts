        ...
        try {
            ...
            for (const a of agents) {
                ...
                rows.push({
                    ...
                    commissionDue,
                    payoutReady: !!(a.bankAccountNumber && a.bankName && a.bankAccountName as any)
                });
            }
            return rows;
        } catch (error) {
...
export const agentVisitService = {
    ...
    async getAllVisits(filters?: {
        ...
        try {
            ...
            if (filters?.agentId) {
                queries.push(Query.equal('agentId', filters.agentId as any));
            }
            if (filters?.providerType) {
                queries.push(Query.equal('providerType', filters.providerType as any));
            }
            if (filters?.membershipAgreed && filters.membershipAgreed !== 'all') {
                queries.push(Query.equal('membershipAgreed', filters.membershipAgreed as any));
            }
            if (filters?.status && filters.status !== 'all') {
                queries.push(Query.equal('status', filters.status as any));
            }
...
+62-XXX-XXXX`;
    ...
    async getMemberLeads(memberId: string, status?: string): Promise<any[]> {
        try {
            ...
            if (status) {
                queries.push(Query.equal('status', status as any));
            }
    ...
    async getAllLeads(filters?: {
        ...
        try {
            ...
            if (filters?.memberType) {
                queries.push(Query.equal('memberType', filters.memberType as any));
            }
            if (filters?.dateFrom) {
                queries.push(Query.greaterThanEqual('sentAt', filters.dateFrom as any));
            }
            if (filters?.dateTo) {
                queries.push(Query.lessThanEqual('sentAt', filters.dateTo as any));
            }
        ...
        try {
            ...
            for (let i = 1; i <= months; i++) {
                ...
                if (billing.totalLeads > 0) {
                    history.push(billing as any as any);
                }
            }
...
export const paymentConfirmationService = {
    ...
    async getAllPayments(filter?: {
        ...
        try {
            ...
            if (filter?.status) {
                queries.push(Query.equal('status', filter.status as any));
            }
            if (filter?.memberType) {
                queries.push(Query.equal('memberType', filter.memberType as any));
            }
            if (filter?.paymentType) {
                queries.push(Query.equal('paymentType', filter.paymentType as any));
            }
