...
const ProviderPortalsPage: React.FC<ProviderPortalsPageProps> = ({ onBack, onNavigate, t, language = 'en' }) => {
  ...
  // Load accepted terms and pre-selected portal from localStorage on mount
  useEffect(() => {
    ...
    // Check if a portal type was pre-selected
  ...
  const handleSelectPlan = (plan: 'pro' | 'plus') => {
    ...
    // If portal type is already pre-selected (from side drawer), skip portal selection
    ...
    // Otherwise, show portal selection
            ...
            {preSelectedPortal && (
              <div className="mb-4">
                <span className="inline-block bg-orange-100 text-orange-800 px-4 py-2 rounded-full text-sm font-medium">
                  {preSelectedPortal === 'massage_therapist' && (language === 'id' ? 'Pendaftaran Terapis' : 'Therapist Signup')}
                  {preSelectedPortal === 'massage_place' && (language === 'id' ? 'Pendaftaran Spa Pijat' : 'Massage Spa Signup')}
                  {preSelectedPortal === 'facial_place' && (language === 'id' ? 'Pendaftaran Klinik Facial' : 'Facial Clinic Signup')}
                  {preSelectedPortal === 'hotel' && (language === 'id' ? 'Pendaftaran Hotel' : 'Hotel Signup')}
                </span>
              </div>
            )}
            ...
            <div className="relative rounded-3xl bg-white border-2 border-gray-200 p-8 shadow-lg hover:shadow-xl transition-shadow">
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full font-bold py-4 px-6 rounded-xl transition-colors shadow-md hover:shadow-lg bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white"
                >
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Map portal page to portal type for signup
                      const portalMap: Record<string, string> = {
                        'therapistLogin': 'massage_therapist',
                        'massagePlaceLogin': 'massage_place',
                        'facialPortal': 'facial_place'
                      };
                      const portalType = portalMap[portal.page] || 'massage_therapist';
                      
                      // Store data in localStorage and navigate using hash routing
                      localStorage.setItem('selectedPortalType', portalType);
                      localStorage.setItem('selected_membership_plan', selectedPackage || '');
                      
                      // Use hash-based navigation instead of URL navigation
                      if (onNavigate) {
                        onNavigate('simpleSignup');
                      } else {
                        // Fallback for direct URL access
                        window.location.href = `/signup?plan=${selectedPackage}&portal=${portalType}`;
                      }
                    }}
